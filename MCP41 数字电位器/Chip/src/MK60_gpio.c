...
void gpio_init(PTX_n ptx_n, GPIO_MOD ddr, uint8 dat)
{
    ...
    if(ddr)
    {
		...
		else
		{
			if(dat == 0)
        {
          PORTX[ptx]->PCR[ptn] &=~ (PORT_PCR_PS_MASK|PORT_PCR_PE_MASK);         //设置输入下拉
          PORTX[ptx]->PCR[ptn] |= PORT_PCR_PE_MASK;
        }
        else
        {
          PORTX[ptx]->PCR[ptn] &=~ (PORT_PCR_PS_MASK|PORT_PCR_PE_MASK);         //设置输入上拉
          PORTX[ptx]->PCR[ptn] |= (PORT_PCR_PS_MASK|PORT_PCR_PE_MASK);
        }
		}
}
