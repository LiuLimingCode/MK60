...
/*!
 *  @brief      获取ADC多次采样值(不支持B通道)
 *  @param      ADCn_Ch_e    ADC通道
 *  @param      ADC_nbit     ADC精度（ ADC_8bit,ADC_12bit, ADC_10bit, ADC_16bit ）
 *  @param      times        采样次数
 *  @return     采样值
 *  Sample usage:       uint16 var = adc_read(ADC0_SE8, ADC_8bit, 16);
 */
uint16 adc_read(ADCn_Ch adcn_ch, ADC_nbit bit, uint16 times)
{
  ADCn_Ch adcn = (ADCn_Ch)(adcn_ch >> 5) ;
  ...
  while(temp_times --)
  {
		ADCX[adcn]->SC1[0] = ADC_SC1_ADCH(adcn_ch);       //启动转换
		
    while (( ADCX[adcn]->SC1[0] & ADC_SC1_COCO_MASK ) != ADC_SC1_COCO_MASK); //等待转换完成
      result += ADCX[adcn]->R[0];
  }
