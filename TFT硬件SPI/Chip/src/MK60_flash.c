 ...
 */
uint8 FLASH_WriteSector(uint32 SectorNum, uint32 offset, FLASH_WRITE_TYPE data)
{
	uint32 addr;
	uint8 buf[4], ret;
	...
	//设置地址
	FTF->FCCOB1 = (addr & 0x00FF0000) >> 16;
	FTF->FCCOB2 = (addr & 0x0000FF00) >> 8;
	FTF->FCCOB3 = (addr & 0x000000FF);
	//设置数据
	FTF->FCCOB4 = buf[3];
	FTF->FCCOB5 = buf[2];
	FTF->FCCOB6 = buf[1];
...
uint8 FLASH_WriteSectorBuf(uint32 SectorNum, const uint8 *buf, uint32 len, uint32 offset)
{
	uint16 step, ret, i;
	union
	{
